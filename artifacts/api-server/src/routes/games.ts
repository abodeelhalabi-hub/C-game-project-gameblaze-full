...
const chunkUpload = multer({ storage: chunkStorage, limits: { fileSize: 12 * 1024 * 1024 } });
...
router.get("/games/:id", async (req, res) => {
  try {
    ...
    res.json({ ...game, createdAt: game.createdAt.getTime(), fileSize: Number(game.fileSize) });
  } catch {
...
router.get("/games/:id/download", async (req, res) => {
  try {
    ...
    if (objectKey && BUCKET_ID) {
      // Generate a short-lived signed URL (1 hour) and redirect the browser to it.
      // This bypasses Replit's proxy entirely so large files never timeout.
      ...
      // Update download count (fire-and-forget — don't block the redirect)
